...
if(isset($_POST['submit'])){
    ...
    $qry = "UPDATE `student` SET `roll`='$roll',`name`='$username',`course`='$course',`mobile`=$phone,`email`='$email' WHERE id=$id";
    ...
    <div class="container">
        ...
        <div class="row">
            ...
            <div class="col-md-4 col-lg-4 form">
                ...
                <form action="update.php" method="POST" onclick="return validation()">
                    <?php while($res = mysqli_fetch_array($display)){  ?>
                    <div class="form-group my-3">
                        <label for="name">Enter Roll Number</label>
                        <input type="tel" name="roll" id="roll" value="<?php echo $res['roll'] ?>" placeholder="Roll Number" class="form-control" pattern="[0-9]{0,}" title=" Special charcters not allowed.">
                        <span id="roll_error" class="font-weight-bold text-danger"></span>
                        <input type="hidden" name="id" id="id" value="<?php echo $res['id'] ?>">
                    </div>
                    <div class="form-group my-4">
                        <label for="name">Enter Full Name</label>
                        <input type="text" name="name" id="name" value="<?php echo $res['name'] ?>" placeholder="Full Name" class="form-control" pattern="[a-z A-Z ]{0,}" title="Only alphabets are allowed.">
                        <span id="name_error" class="font-weight-bold text-danger"></span>
                    </div>
                    <div class="form-group my-4">
                        <label for="course">Enter Course</label>
                        <input type="text" name="course" id="course" value="<?php echo $res['course'] ?>" placeholder="Course" class="form-control" pattern="[a-z A-Z.]{0,}" title="Special charcters not allowed.">
                        <span id="course_error" class="font-weight-bold text-danger"></span>
                    </div>
                    <div class="form-group my-4">
                        <label for="phone">Enter Phone</label>
                        <input type="tel" name="phone" id="phone" value="<?php echo $res['mobile'] ?>" placeholder="Phone" class="form-control">
                        <span id="phone_error" class="font-weight-bold text-danger"></span>
                    </div>
                    <div class="form-group my-4">
                        <label for="email">Enter Email</label>
                        <input type="email" name="email" id="email" value="<?php echo $res['email'] ?>" placeholder="Email" class="form-control">
                        <span id="email_error" class="font-weight-bold text-danger"></span>
                    </div>
                    <button class="btn btn-primary rounded-0" name="submit">Update</button>
                </form>   
                    <?php } ?>   
            </div>
            <div class="col-md-4 col-lg-4">
